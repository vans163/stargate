...
->
    %We got an error parsing the request line, netscanner or io error
    {stop, {shutdown, tcp_closed}, D};
...
handle_event(info, {ws_message, Msg}, websocket, D=#{ws_handler:= WSHandler}) ->
    ...
    try
        ...
        E:R -> ?PRINT({"cast apply failed", Msg, E, R})
    end,
...
forward_ws_closed(_D=#{ws_handler:= WSHandler, temp_state:= TempState}) ->
